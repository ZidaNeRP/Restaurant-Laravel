<section>
    ...
    <form method="post" action="{{ route('user.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="email" name="email" type="email" class="mt-1 block w-full" :value="old('email', $role->email)" required autocomplete="rolename" />
            <x-input-error class="mt-2" :messages="$errors->get('email')" />

            @if ($role instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $role->hasVerifiedEmail())
                <div>
